import torch
import numpy
from alvg.models import MODELS
...
class MIXDETRMB(OneStageModel):
    ...
    def forward_train(
        ...
        losses_dict, output = self.head.forward_train(
            img_feat, img_metas, cls_feat=cls_feat, gt_bbox=gt_bbox, gt_mask=gt_mask, text_feat=text_feat, text_mask=text_attention_mask
        )
        ...
        with torch.no_grad():
            if img_metas[0].get("target", None) is None:
                ...
                predictions_decoder_branch = self.get_predictions_grec(output_decoder_branch, img_metas,
                                                                       rescale=rescale)
    ...
    def forward_test(
        ...
        output = self.head.forward_test(img_feat, img_metas, text_feat=text_feat, cls_feat=cls_feat,
                                        with_bbox=with_bbox, with_mask=with_mask, text_mask=text_attention_mask)
        ...
        return predictions, output['attn_map'], output['score']
    ...
    def get_predictions(self, output, img_metas, rescale=False):
        ...
        predict_classes = []
        for results_per_image, mask_per_image, img_meta in zip(results, mask_pred, img_metas):
            ...
            mask_per_image = F.interpolate(mask_per_image.unsqueeze(0).unsqueeze(0), size=(height, width), mode='bilinear', align_corners=False).squeeze(0)
            pred_masks.append(
               mask_per_image
            )
    ...
    def get_predictions_grec(self, output, img_metas, rescale=False):
        ...
        for results_per_image, mask_per_image, img_meta in zip(results, mask_pred, img_metas):
            ...
            mask_per_image = F.interpolate(mask_per_image.unsqueeze(0).unsqueeze(0), size=(height, width), mode='bilinear', align_corners=False).squeeze(0).squeeze(0)

            pred_masks.append(
                {
                    "masks": mask_per_image,
                }
            )
            # processed_results.append({"instances": r})
