...
class SwinTransformerMM(BaseModule):
    """Swin Transformer
    ...
    def __init__(
        ...
        super(SwinTransformerMM, self).__init__(init_cfg=init_cfg)
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformerMM, self).train(mode)
        self._freeze_stages()
    ...
    def forward(self, x, y):
        ...
        if self.use_abs_pos_embed:
            ...
            if hw_shape[0] != h or hw_shape[1] != w:
                absolute_pos_embed = (
                    F.interpolate(self.absolute_pos_embed, size=hw_shape, mode="bicubic", align_corners=False).flatten(2).transpose(1, 2)
                )
            else:
