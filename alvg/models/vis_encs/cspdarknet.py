...
from alvg.models.utils import freeze_params, parse_yolo_weights
...
class Focus(nn.Module):
    ...
    def __init__(
        ...
        self.conv = ConvModule(
            in_channels * 4, out_channels, kernel_size, stride, padding=(kernel_size - 1) // 2, conv_cfg=conv_cfg, norm_cfg=norm_cfg, act_cfg=act_cfg
        )
...
class CSPDarknet(BaseModule):
    ...
    arch_settings = {
        "P5": [[64, 128, 3, True, False], [128, 256, 9, True, False], [256, 512, 9, True, False], [512, 1024, 3, False, True]],
        "P6": [
            [64, 128, 3, True, False],
    ...
    def __init__(
        ...
        if frozen_stages not in range(-1, len(arch_setting) + 1):
            raise ValueError("frozen_stages must be in range(-1, " "len(arch_setting) + 1). But received " f"{frozen_stages}")
