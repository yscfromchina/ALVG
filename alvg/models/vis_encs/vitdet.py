    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            if isinstance(self.init_cfg.get("prefix", None),str):
                ...
                _state_dict = {key.replace(prefix, ""):value for key,value in _state_dict.items() if prefix in key}
            # load_state_dict(self, state_dict, strict=False, logger=logger)
            missing_keys, unexpected_keys = self.load_state_dict(_state_dict, strict=False)
            if is_main():
