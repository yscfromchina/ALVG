...
from mmcv.cnn import Conv2d, build_activation_layer, build_norm_layer, constant_init, normal_init, trunc_normal_init
...
from alvg.models import VIS_ENCODERS
from alvg.utils import get_root_logger, is_main
from mmcv.runner import force_fp32

from alvg.core.layers import MLP
...
class SpatialReductionAttention(MultiheadAttention):
    ...
    def __init__(
        ...
        super().__init__(
            embed_dims, num_heads, attn_drop, proj_drop, batch_first=batch_first, dropout_layer=dropout_layer, bias=qkv_bias, init_cfg=init_cfg
        )
    ...
    def forward(self, x, y, hw_shape, identity=None, identity_y=None):
        x_q = x
        y_q = y
        if self.sr_ratio > 1:
        ...
        if identity is None or identity_y is None:
            identity = x_q
            identity_y = y_q
        ...
        if self.batch_first:
            ...
            y_q = y_q.transpose(0, 1)

        # fusion type one
        y_q = self.attn(query=y_q, key=x_kv, value=x_kv)[0]
        x_q = torch.concat((x_q, y_q), dim=0)
        ...
        x_q = identity + out[:, :-y.shape[1]]
        y_q = identity_y + out[:, -y.shape[1]:]
        
        # fusion type two
        # y_q = self.attn(query=y_q, key=x_kv, value=x_kv)[0]
        # x_q = self.attn(query=x_q, key=y_q, value=y_q)[0]
        # out = self.attn(query=x_q, key=x_kv, value=x_kv)[0]
        ...
        # out = self.dropout_layer(self.proj_drop(out))
        # y_q = self.dropout_layer(self.proj_drop(y_q))
        # x_q = identity + out
        # y_q = identity_y + y_q
...
class PVTEncoderLayer(BaseModule):
    ...
    def forward(self, x, y, hw_shape):
        x, y = self.attn(self.norm1(x),self.norm1(y), hw_shape, identity=x, identity_y=y)
...
class AbsolutePositionEmbedding(BaseModule):
    ...
    def __init__(self, pos_shape, pos_dim, drop_rate=0.0, init_cfg=None):
        ...
        if isinstance(pos_shape, int):
            ...
            assert len(pos_shape) == 2, f"The size of image should have length 1 or 2, " f"but got {len(pos_shape)}"
...
class PyramidVisionTransformerMMMix(BaseModule):
    ...
    def __init__(
        ...
        # transformer encoder
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(num_layers))]  # stochastic num_layer decay rule

        cur = 0
        self.layers = ModuleList()
        self.text_embedding = ModuleList()
        self.output_dims = []
        for i, num_layer in enumerate(num_layers):
            ...
            text_embedding = MLP(text_in_channels, embed_dims_i, embed_dims_i)
            if i in out_indices:
                self.output_dims.append(embed_dims_i)
            # text_embedding = nn.Linear(text_in_channels, embed_dims_i)
            ...
            text_in_channels = embed_dims_i
            # The ret[0] of build_norm_layer is norm name.
            if norm_after_stage:
                norm = build_norm_layer(norm_cfg, embed_dims_i)[1]
            else:
                norm = nn.Identity()
            self.text_embedding.append(text_embedding)
    ...
    def train(self, mode=True):
        ...
        super(PyramidVisionTransformerMMMix, self).train(mode)
        self._freeze_stages()
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            logger.warn(f"Load pre-trained model for " f"{self.__class__.__name__} from original repo")
            ...
            if isinstance(self.init_cfg.get("prefix", None), str):
                prefix = self.init_cfg["prefix"]
                state_dict = {key.replace(prefix, ""): value for key, value in state_dict.items() if prefix in key}
            if len(self.text_embedding)>0:
                self.text_embedding.apply(self.init_modules)
                if is_main():
                    logger.info("initial text embedding success!!")

            # load_state_dict(self, state_dict, strict=False, logger=logger)
            missing_keys, unexpected_keys = self.load_state_dict(state_dict, strict=False)
            if is_main():
                logger.info("Load pretrained backbone checkpoint from:{}".format(str(self.init_cfg.checkpoint)))
    ...
    def forward(self, x, y):
        ...
        for i, layer in enumerate(self.layers):
            ...
            y = self.text_embedding[i](y)
            
            for block in layer[1]:
                x, y = block(x, y, hw_shape)
...
@VIS_ENCODERS.register_module()
class PyramidVisionTransformerV2MMMix(PyramidVisionTransformerMMMix):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PyramidVisionTransformerV2MMMix, self).__init__(
            patch_sizes=[7, 3, 3, 3], paddings=[3, 1, 1, 1], use_abs_pos_embed=False, norm_after_stage=True, use_conv_ffn=True, **kwargs
        )
