...
class Resize(object):
    ...
    def _resize_mask(self, results):
        if results["with_mask"]:
            if self.keep_ratio:
                if isinstance(results["gt_mask"], list):
                    results["gt_mask"] = [mask.rescale(results["scale"]) for mask in results["gt_mask"]]
                else:
                    results["gt_mask"] = results["gt_mask"].rescale(results["scale"])
            else:
                if isinstance(results["gt_mask"], list):
                    results["gt_mask"] = [mask.resize(results["img_shape"][:2]) for mask in results["gt_mask"]]
                else:
                    results["gt_mask"] = results["gt_mask"].resize(results["img_shape"][:2])
            if isinstance(results["gt_mask"], list):
                results["gt_mask_rle"] = [maskUtils.encode(numpy.asfortranarray(mask.masks[0])) for mask in results["gt_mask"]]
            else:
...
class Pad:
    ...
    def _pad_masks(self, results):
        ...
        if results["with_mask"]:
            ...
            if isinstance(results["gt_mask"], list):
                results["gt_mask"] = [mask.pad(pad_shape, pad_val=self.pad_val) for mask in results["gt_mask"]]
                results["gt_mask_rle"] = [maskUtils.encode(numpy.asfortranarray(mask)) for mask in results["gt_mask"]]
            else:
