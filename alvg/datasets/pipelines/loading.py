...
class LoadImageAnnotationsFromFile(object):
    ...
    def _load_masks(self, results):
        if self.with_mask:
            masks = results["ann"]["segmentation"][self.random_ind]
            h, w = results["ori_shape"][:2]
            all_rles = []
            all_decoded_masks = []
            is_crowd = []
            if not any(masks):
                all_zeros_masks = np.zeros((1, h, w), dtype=np.uint8)
                mask = BitmapMasks(all_zeros_masks, h, w)
                results["gt_mask"] = mask
                results["gt_mask_rles"] = []
                results["is_crowd"] = []
            else:
                for mask in masks:
                    ...
                    try:
                        ...
                        print(f'rle: {rle}')
                    all_rles.append(rle)
                    all_decoded_masks.append(decoded_mask)

                all_decoded_masks = np.array(all_decoded_masks)
                mask = BitmapMasks(all_decoded_masks, h, w)

                results["gt_mask"] = mask
                results["gt_mask_rles"] = all_rles
                results["is_crowd"] = is_crowd
    ...
    def __init__(
        ...
        if use_token_type == "bert":
            ...
            corpus_path = osp.join(
                "/home/d/demo_mirror/REC/vgtr/store/data/unc", "corpus.pth"
            )
            self.corpus = torch.load(corpus_path)
        elif use_token_type == "beit3":
            self.tokenizer = XLMRobertaTokenizer("/home/d/demo_mirror/vlm/unilm/beit3/pretrain_weights/beit3.spm")
            self.bos_token_id = self.tokenizer.bos_token_id
            self.eos_token_id = self.tokenizer.eos_token_id
            self.pad_token_id = self.tokenizer.pad_token_id
