...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        matcher,
        weight_dict,
        losses: List[str] = ["class", "boxes", "masks"],
        eos_coef: float = 0.1,
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        if targets[0]['masks'] is None:
            losses = {
                "loss_mask": torch.tensor(0, device=src_masks.device),
                "loss_dice": torch.tensor(0, device=src_masks.device),
            }
            return losses
