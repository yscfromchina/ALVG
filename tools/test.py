...
def main_worker(cfg):
    ...
    for eval_loader, _prefix in zip(dataloaders, prefix):
        if is_main():
            logger = get_root_logger()
            logger.info(f"ALVG - evaluating set {_prefix}")
        evaluate_model(-1, cfg, model, eval_loader)
        if cfg.ema:
            if is_main():
                logger = get_root_logger()
                logger.info(f"ALVG - evaluating set {_prefix} using ema")
...
def parse_args():
    parser = argparse.ArgumentParser(description="ALVG-test")
    # parser.add_argument('config', help='test configuration file path.')
    parser.add_argument('--config', default="configs/single/ViT-base/grefcoco/grefcoco.py", help='test configuration file path.')
    parser.add_argument(
        '--load-from', default="work_dir/single/ViT-base/grefcoco/grefcoco_twostage_1/baseline/det_best.pth", help='load from the saved .pth checkpoint, only used in validation.')
    parser.add_argument(
