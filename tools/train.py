...
import sys
from alvg.core import build_optimizer, build_scheduler
from alvg.datasets import build_dataset, build_dataloader
from alvg.models import build_model, ExponentialMovingAverage
from alvg.apis import set_random_seed, train_model, evaluate_model
from alvg.utils import get_root_logger, load_checkpoint, save_checkpoint, load_pretrained_checkpoint, is_main, init_dist
...
def parse_args():
    ...
    parser.add_argument("--config", required=False, default="configs/single/ViT-base/grefcoco/grefcoco.py", help="training configuration file path.")
...
def main_worker(cfg):
    ...
    if cfg.resume_from:
        start_epoch, _, _, _ = load_checkpoint(model, model_ema, cfg.resume_from, amp=cfg.use_fp16, optimizer=optimizer, scheduler=scheduler)
