...
from alvg.datasets.builder import build_dataset
import cv2
import mmcv
import numpy as np
...
from alvg.utils.det_cam_visualizer import DetAblationLayer, DetBoxScoreTarget, DetCAMModel, DetCAMVisualizer, EigenCAM, FeatmapAM, reshape_transform
...
def parse_args():
    ...
    parser.add_argument(
        "--target-layers",
        default=["vis_enc.beit3.encoder.layers[6]"],
...
def main():
    ...
    for image_path in images:
        ...
        if args.method in GRAD_BASE_METHOD_MAP:
            ...
            bbox = deepcopy(bboxes[0])
            # bbox = bboxes[0]
            bbox[2] = bbox[2] - bbox[0]
            bbox[3] = bbox[3] - bbox[1]
            model.set_input_data(image_path, text, bboxes=bbox, labels=labels)
        ...
        image_with_bounding_boxes = det_cam_visualizer.show_cam(image, bboxes, labels, grayscale_cam, False, draw_boxes=False)
